...
import net.cnbec.catering.ui.main.fragment.SCFragment;
...
public class NewSCActivity extends BaseActivity<BaseContractCode.View, BaseContractCode.Presenter> implements BaseContractCode.View {
    ...
    public static void startActivity(Context context) {
        Intent intent = new Intent(context, NewSCActivity.class);
//        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);//后期放开
    ...
    protected int getLayoutId() {
        return R.layout.activity_sc_new;
    }
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        fragments.add(SCFragment.newInstance(0));
        titles.add("杂交收藏");
        fragments.add(SCFragment.newInstance(1));
        titles.add("种质收藏");
    ...
    public void onMoonStickyEvent(Map<String,Object> map){
        ...
        if(option.equals("scSelectYear")){
            for(int i = 0; i < fragments.size(); i++) {
                Fragment fragment = fragments.get(i);
                if(fragment != null && fragment.isAdded() && fragment.isMenuVisible()) {
                    // ToastUtil.show(i+"");
                    if(i == 0){
                        ((SCFragment)fragment).onMoonStickyEvent(map);
                    }else if(i == 1){
                        ((SCFragment)fragment).onMoonStickyEvent(map);
                    }
                }
