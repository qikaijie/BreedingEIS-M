...
@interface ZZDetailsViewController ()<UITableViewDelegate,UITableViewDataSource,NFCNDEFReaderSessionDelegate>{
    Boolean isCollect;
...
@property (strong, nonatomic) NSMutableDictionary *seedlingInfoBean;
...
@implementation ZZDetailsViewController
...
- (void)queryCodeInfo:(NSString *)code{
    [GJHttpTool GET:APP_SEEDLING_INFO parameters:@{@"code":code} headerParams:nil awaitingView:self.view success:^(id responseObject) {
        /**
         "germplasmId" : 94,
        ...
        if([[responseObject objectForKey:@"code"] intValue] == 200){
           ...
           [self.sumbitBtn setBackgroundColor:navigationBarColor];
            ...
            //赋值列表
            self.seedlingInfoBean = [[NSMutableDictionary alloc]init];
            self.seedlingInfoBean = [[responseObject objectForKey:@"data"] mutableCopy];
            self.scanTf.text = [NSString stringWithFormat:@"%@ %@",[self.seedlingInfoBean objectForKey:@"code"],[self.seedlingInfoBean objectForKey:@"germplasmName"]];
            bool isCollect = [[self.seedlingInfoBean objectForKey:@"isCollect"] boolValue];
            
            //是否被收藏
            [self.collectBtn setImage:[UIImage imageNamed:(isCollect ? ([[self.seedlingInfoBean objectForKey:@"collectLevel"] intValue]==1 ? @"icon_sc_s" : @"icon_sc_ss") : @"icon_sc")] forState:UIControlStateNormal];
...
- (IBAction)collectBtnOnClick:(UIButton *)sender {
    if(self.seedlingInfoBean != nil){
        NSMutableDictionary *params = [[NSMutableDictionary alloc]init];
        [params setObject:[NSNumber numberWithInt:[[self.seedlingInfoBean objectForKey:@"id"] intValue]] forKey:@"seedlingId"];
        bool isCollect = [[self.seedlingInfoBean objectForKey:@"isCollect"] boolValue];
        if(isCollect){
            //判断是不是二级收藏
            if([[self.seedlingInfoBean objectForKey:@"collectLevel"] intValue] == 2){
                //取消收藏
                [GJHttpTool GET:APP_SEEDLING_COLLECT_DEL parameters:@{@"id":[NSNumber numberWithInteger:[[self.seedlingInfoBean objectForKey:@"collectId"] intValue]]} headerParams:@{} awaitingView:self.view success:^(id responseObject) {
                    if([[responseObject objectForKey:@"code"] intValue] == 200){
                        //没有收藏
                        [SVProgressHUD showSuccessWithStatus:NSLocalizedString(@"cancel.collect", @"")];
                        self->isCollect = YES;
                        [self queryCodeInfo:[self.seedlingInfoBean objectForKey:@"code"]];
                    }else{
                        [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"%@",[responseObject objectForKey:@"msg"]]];
                    }
                } failure:^(NSError *error) {
                ...
                [GJHttpTool POST:APP_SEEDLING_COLLECT_ADD parameters:params headerParams:@{} awaitingView:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        [self.view toast:NSLocalizedString(@"qxsmxxzzzh", @"")];
    }
}
...
- (IBAction)prevCodeOnClick:(UIButton *)sender {
    if(self.seedlingInfoBean != nil){
        if(!strIsEmpty([self.seedlingInfoBean objectForKey:@"prevCode"])){
            if(!self->isUploadResult){
                ...
                [alertController addAction:([UIAlertAction actionWithTitle:NSLocalizedString(@"out", @"") style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
                    ...
                    [self queryCodeInfo:[self.seedlingInfoBean objectForKey:@"prevCode"]];
                }])];
...
- (IBAction)nextCodeOnClick:(UIButton *)sender {
    if(self.seedlingInfoBean != nil){
        if(!strIsEmpty([self.seedlingInfoBean objectForKey:@"nextCode"])){
            if(!self->isUploadResult){
                ...
                [alertController addAction:([UIAlertAction actionWithTitle:NSLocalizedString(@"out", @"") style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
                    self->isUploadResult = false;
                    [self queryCodeInfo:[self.seedlingInfoBean objectForKey:@"nextCode"]];
                }])];
                [alertController addAction:([UIAlertAction actionWithTitle:NSLocalizedString(@"sendnow", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [self sumbitDataOnClick:self.sumbitBtn];
                }])];
...
- (IBAction)makePhotoOnClick:(UIButton *)sender {
    if(self.seedlingInfoBean != nil){
        ...
        updatingFilesViewController.plantCode = [NSString stringWithFormat:@"%@",[self.seedlingInfoBean objectForKey:@"code"]];
        updatingFilesViewController.plantId = [NSString stringWithFormat:@"%d",[[self.seedlingInfoBean objectForKey:@"id"] intValue]];
...
- (IBAction)recordOnClick:(UIButton *)sender {
    if(self.seedlingInfoBean != nil){
        ZZHistoryViewController *historyViewController = [[ZZHistoryViewController alloc]init];
        historyViewController.title = NSLocalizedString(@"his.record", @"");
        historyViewController.seedlingId = [[self.seedlingInfoBean objectForKey:@"id"] intValue];
...
- (IBAction)sumbitDataOnClick:(id)sender {
    if(self.seedlingInfoBean != nil){
        ...
        [dataDict setObject:[NSString stringWithFormat:@"%d",[[self.seedlingInfoBean objectForKey:@"germplasmId"] intValue]] forKey:@"germplasmId"];
        [dataDict setObject:[NSString stringWithFormat:@"%d",[[self.seedlingInfoBean objectForKey:@"id"] intValue]] forKey:@"seedlingId"];
        ...
        [GJHttpTool POST:APP_SEEDLING_RECORD_ADD parameters:dataDict headerParams:@{} awaitingView:self.view success:^(id responseObject) {
            if([[responseObject objectForKey:@"code"] intValue] == 200){
