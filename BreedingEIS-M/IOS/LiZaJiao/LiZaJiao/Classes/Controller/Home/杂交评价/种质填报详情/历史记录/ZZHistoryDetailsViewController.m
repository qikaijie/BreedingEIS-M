//
//  ZZHistoryDetailsViewController.m
//  LiZaJiao
//
...
@interface ZZHistoryDetailsViewController ()<UITableViewDelegate,UITableViewDataSource>{
    int groupIndex;
    int seedlingId;
    int selectIndex;
}
...
@implementation ZZHistoryDetailsViewController
...
- (void)queryData{
    [GJHttpTool GET:APP_SEEDLING_RECORD_HISTORY_LIST_INFO parameters:@{@"seedlingId":[NSNumber numberWithInt:self->seedlingId],@"type":[NSNumber numberWithInt:self->groupIndex+1]} headerParams:@{} awaitingView:self.view success:^(id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        ...
        historyDetailsTopCell.nameTitleLabel.text = NSLocalizedString(@"zzzzmc", @"");
        historyDetailsTopCell.codeLabel.text = [NSString stringWithFormat:@"%@",[tempDict objectForKey:@"seedlingCode"]];
        historyDetailsTopCell.nameLabel.text = [NSString stringWithFormat:@"%@",[tempDict objectForKey:@"germplasmName"]];
