...
@Api(tags = "移动端-种质信息的植物图片、视频上传管理")
@RestController
@RequestMapping(value="/app/v10/seedling/upload")
public class SeedlingUploadController extends APPBaseController{
	
	private static final Logger logger = LoggerFactory.getLogger(SeedlingUploadController.class);
	...
	@PostMapping(value = "/file/{seedlingId}", produces = "application/json;charset=utf-8")
	@ResponseBody
	public AjaxResult uploadFile(MultipartFile file, @PathVariable("seedlingId")Long seedlingId) {
		if (!file.isEmpty()) {
			try {
				// 判断文件的MIMEtype
				...
				Seedling seedling = iSeedlingService.getById(seedlingId);
				if(seedling == null)
				{
					logger.error("植物信息不存在，seedlingId:{}", seedlingId);
				...
				uploadPath.append(seedling.getCodeTwo());
